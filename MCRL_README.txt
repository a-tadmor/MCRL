...
3. If there is a problem downloading the blast software or RefSeq files make sure you have an internet connection and that the firewall is not blocking the ftp port.

4. If there is a problem downloading the blast software, blast v2.2.22+ can be downloaded manually from the NCBI website: ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.2.22/, selecting the software version appropriate for your OS. 
...
5. If there is a problem running blast there may be a previous installation of blast from another utility which is interfering with MCRL. In you are running Windows and Windows is installed on the c:\ drive, check the Windows folder for a file called ncbi.ini and temporarily rename it.
