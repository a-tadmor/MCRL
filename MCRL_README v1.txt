...
Part I: 
1. Download and extract MCRL_WIN64.zip to a local (offline) folder, for example the C:\ drive
2. Double click the executable MCRL_installer_WIN64.exe and follow the installation instructions to install the "MATLAB Compiler Runtime" (MCR) and the MCRL software locally.

Part II: install MCRL
3. Navigate to the "application" folder and double click the executable MCRL.exe
...
To produce reference gene networks plots double click the executable "Plot_networks_installer_WIN64.exe" and follow the installation instructions.
For instructions how to use this application see the Supporting Information section.
...
INSTALLATION INSTRUCTIONS:
1. Download and extract MCRL_Matlab.zip locally
2. Start up Matlab 
...
2. Make sure to download and extract Windows executables to a local offline folder, such as the c:\ drive

3. If there is a problem downloading the blast software or RefSeq files make sure you have an internet connection.

4. If your internet connection is working properly, try temporarily disabling your firewall. Your firewall may be blocking an "active" ftp connection.

5. If there is a problem downloading the blast software, blast v2.2.22+ can be downloaded manually from the NCBI website: ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.2.22/, selecting the software version appropriate for your OS. 
...
6. If there is a problem running blast there may be a previous installation of blast from another utility which is interfering with MCRL. In you are running Windows and Windows is installed on the c:\ drive, check the Windows folder for a file called ncbi.ini and temporarily rename it.
